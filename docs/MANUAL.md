...
목차 0. 로그인 1. 개요 2. 최초 준비(한 번만) 3. 카페 설정 4. 키워드 구성 5. 작업 추가 6. 이미지 7. 별명(자동 변경) 8. 댓글만 작업 9. 작업 시작 · 예약 발행 10. 글 스타일(디자인) 11. 문제 해결 12. 배포(exe)
...
## 8. 댓글만 작업 — 글은 안 쓰고 댓글만 달기

![08_댓글작업](comment_only.png)

▲ 댓글만 작업 탭 — 카페 자동 스캔 / 글 주소 직접 지정
...
여러 카페를 돌릴 때 — [카페 섞기] 카페A 100개, 카페B 100개를 담으면 기본은 A를 100개 연속으로 올린 뒤 B로 넘어갑니다. 한 카페에 연속으로 많이 쓰면 연속 등록 제한에 걸리기 쉽습니다. ‘작업 목록’ 아래 [카페 섞기] + 숫자를 쓰면 카페별로 그 개수만큼 번갈아 재배치합니다. 예) 2를 넣으면 A A B B A A B B … 순서가 됩니다. (카페가 3개 이상이어도 순서대로 번갈아 돌고, 한 카페가 먼저 소진되면 남은 카페 작업이 이어집니다)
